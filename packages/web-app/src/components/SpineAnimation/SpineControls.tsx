...
export const SpineControls: React.FC<SpineControlsProps> = ({
  ...
  return (
    <Box sx={{ 
      ...
      {isActive && (
        <>
          <Tooltip title={`Switch to ${currentCharacterSet === 'pengu' ? 'Capybara' : 'Pengu'}`} placement="top">
            <IconButton
          ...
          <Tooltip title={`${isSoundEnabled ? 'Mute' : 'Turn On'}`} placement="top">
            <IconButton
      ...
      <Tooltip title={`${isActive ? 'Hide' : 'Show'}`} placement="top">
        <IconButton
